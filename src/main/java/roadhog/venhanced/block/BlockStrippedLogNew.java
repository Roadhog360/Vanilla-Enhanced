...
public class BlockStrippedLogNew extends BlockLog {

	public static final String[] icons = new String[] {"Acacia", "BigOak"};
	...
	public static void onPlayerInteract(PlayerInteractEvent event) {
		...
		ItemStack drop = new ItemStack(VanillaEnhanced.blockBark2, 1, logMeta & 3);
		EntityItem entityitem = new EntityItem(world, event.x + 0.5, event.y + 0.5, event.z + 0.5, drop);
			...
			if (event.entityPlayer != null && !player.isSneaking()) {
				if (event.action == Action.RIGHT_CLICK_BLOCK)
					if (world.getBlock(event.x, event.y, event.z) == Blocks.log2) {
						ItemStack stack = event.entityPlayer.getCurrentEquippedItem();
						if (stack != null && (stack.getItem() instanceof ItemAxe)) {
							world.setBlock(event.x, event.y, event.z, VanillaEnhanced.blockStrippedLog2, logMeta, 2);
