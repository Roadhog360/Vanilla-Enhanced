...
public class ItemBlaziciteSpade extends ItemSpade {

	public ItemBlaziciteSpade(ToolMaterial material) {
		super(material);
	}
	
	public boolean getIsRepairable(ItemStack par1ItemStack, ItemStack par2ItemStack){
		if(par1ItemStack.getItem() == VanillaEnhanced.itemBlaziciteSpade) {
	...
	public boolean onBlockStartBreak(ItemStack itemstack, int i, int j, int k, EntityPlayer player)
	{
		...
		if(this == VanillaEnhanced.itemBlaziciteSpade && !player.capabilities.isCreativeMode)
		{
...
//		            if (pitchSound > 1) pitchSound = 1;
					...
					if(block == Blocks.clay) {
						drop = new ItemStack(Items.brick, random.nextInt(level + 1) + 1);
					}
					if (fxplay == 0) {
					...
					if(!world.isRemote)
					{
						...
						try {
							...
							while(exp > 0) {
								...
								exp -= expAmount;
								if(block.getMaterial() == Material.sand || block == Blocks.clay) {} else {
