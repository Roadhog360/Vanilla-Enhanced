...
public class ItemBlaziciteAxe extends ItemAxe {

	public ItemBlaziciteAxe(ToolMaterial material) {
		super(material);
	}
	
	public boolean getIsRepairable(ItemStack par1ItemStack, ItemStack par2ItemStack){
		if(par1ItemStack.getItem() == VanillaEnhanced.itemBlaziciteAxe) {
	...
	public boolean onBlockStartBreak(ItemStack itemstack, int i, int j, int k, EntityPlayer player)
	{
		...
		if(this == VanillaEnhanced.itemBlaziciteAxe && !player.capabilities.isCreativeMode)
		{
			...
			if(block != null && this.getDigSpeed(itemstack, block, meta) > 1.0F)
			{
				for (ItemStack item : items)
				{
		            ...
		            int var3 = random.nextInt(level + 1);
					...
					if(!world.isRemote)
					{
						...
						try {
							...
							while(exp > 0) {
								...
								exp -= expAmount;
								if(drop.getItem() == Items.coal) {} else {
